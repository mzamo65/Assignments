...
public class ShopGrid {
	...
	private Semaphore enter, space, empty, move;
	private final int x;
	private final int y;
	private int max=0;
	...
	ShopGrid(int x, int y, int [][] exitBlocks, int maxPeople) throws InterruptedException {
		...
		this.x=x;
		this.max = maxPeople;
		score = new PeopleCounter(max);
		this.y=y;
		this.checkout_y=y-3;
		Blocks = new GridBlock[x][y];
		this.initGrid(exitBlocks);
		enter = new Semaphore(1);
		space = new Semaphore(0);
	...
	public GridBlock enterShop() throws InterruptedException {
		...
		if (empty.availablePermits() > 0){
			space.acquire();  // wait for customer to exit
			move.acquire();  // wait for entry block to be released
			enter.acquire();
	...
	public synchronized GridBlock move(GridBlock currentBlock,int step_x, int step_y) throws InterruptedException {
		...
		if ((new_x == entrance.getX())&&(new_y == entrance.getY())) // Moving back to entrance
			return currentBlock;
